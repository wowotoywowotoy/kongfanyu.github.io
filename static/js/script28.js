(function($) {
    ...
    if ($('html').hasClass('is-builder')) return;
  ...
  //***************************** mbr-shop ***********************************//
    ...
    var rangeSliderInit = function(context) {
      ...
      $(window).on('resize', function() {
          ...
          var widthRangeWrap = rangeWrap.width() - 20;
          var widthOne = (constRight - constLeft) / widthRangeWrap;

          maxRange    = widthRangeWrap;

          rangeLeft   = Math.ceil((valLeft - constLeft) / widthOne);
          rangeRight  = Math.ceil((valRight - constLeft) / widthOne);

          toggleMin.css({left: function(i, v) {
            return rangeLeft;
          }});
          toggleMax.css({right: function(i, v) {
            return widthRangeWrap - rangeRight + 1;
          }});
    ...
    var priceSliderInit = function(context) {
      $(context).find('input[name=max]').val(findMaxItemPrice(context));
      $(context).find('input[name=min]').val(findMinItemPrice(context));
